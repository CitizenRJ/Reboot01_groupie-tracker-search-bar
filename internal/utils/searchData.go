...
func SearchData(searchTerm string, searchType int, allData gpd.Data) gpd.Data {
	...
	if searchType == 0 {
		dataSearch = searchByTerm(searchTerm, allData, bandMembers)
	} else {
		dataSearch = searchByCreationDate(searchType, allData, bandMembers)
	}
...
func searchByTerm(searchTerm string, allData gpd.Data, bandMembers map[string]bool) gpd.Data {
	var dataSearch gpd.Data
	for i := 0; i < len(allData.Artist); i++ {
		...
		if matchesSearchTerm(searchTerm, artist, artistName, firstAlbum, allData, i, bandMembers) {
			dataSearch.Artist = append(dataSearch.Artist, artist)
			updateBandMembers(artist, bandMembers)
		}
	}
	return dataSearch
}

func searchByCreationDate(searchType int, allData gpd.Data, bandMembers map[string]bool) gpd.Data {
	var dataSearch gpd.Data
	for i := 0; i < len(allData.Artist); i++ {
		artist := allData.Artist[i]
...
func matchesSearchTerm(searchTerm string, artist gpd.Artists, artistName, firstAlbum string, allData gpd.Data, i int, bandMembers map[string]bool) bool {
...
func updateBandMembers(artist gpd.Artists, bandMembers map[string]bool) {
	for _, member := range artist.Members {
